...
# CFLAGS   := -Wall -g -D_GNU_SOURCE -std=c11 $(INCDIRS) -I/users/NewB/gam/include
# # -rdynamic makes main binary symbols visible to dlsym()
# # --no-as-needed prevents linker from dropping transitive UCX libs
# LDFLAGS  := $(LIBDIRS) $(UCX_LINK) -ldl -lpthread -rdynamic -Wl,--no-as-needed -Wl,-rpath,$(UCX_LIBDIR) -L/users/NewB/gam/src -ldsm
...
# # Apps program (add more apps here)
# APPS     := matrix_multiply

# APP_SRCS := $(addprefix $(APP_DIR)/,$(addsuffix .c,$(APPS)))
# APP_OBJS := $(APP_SRCS:.c=.o)
# APP_BINS := $(addprefix $(BIN_DIR)/,$(APPS))

# # Default target: build all demos
# all: $(BIN_DIR) $(LOG_DIR) $(DEMO_BINS) $(APP_BINS)

# # ===== Link rules for demos =====
# $(BIN_DIR)/%: $(TEST_DIR)/%.o $(OBJS) | $(BIN_DIR)
# 	$(CC) -o $@ $^ $(LDFLAGS)

# $(BIN_DIR)/%: $(APP_DIR)/%.o $(OBJS) | $(BIN_DIR)
# 	$(CC) -o $@ $^ $(LDFLAGS)
